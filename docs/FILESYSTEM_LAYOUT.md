...
representation for the session. This directory includes an `agent.sock` symlink.
We use a symlink because Unix-domain socket path lengths are platform-limited;
for example, macOS `unix(4)` documents `sockaddr_un.sun_path` as 104 bytes:
https://manpages.org/unix/4

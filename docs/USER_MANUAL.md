...
`--alias` defaults to the first segment of its GUID. A remote session created
without `--alias` gets a short `r...` alias generated by the remote host. After
you attach to a remote session, sessh caches a local route so later

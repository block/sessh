...
- `--attach [ID]`: attach to an existing session. `ID` may be an alias or a
  session GUID. With `HOST`, aliases are resolved on that host. Without `HOST`,
  aliases are resolved from the local state directory. Without an id, attach to
  the most recent attachable session.
...
`sessh --attach ALIAS` can reconnect without restating the host. If
`sessh HOST --attach ALIAS` resolves to a route for another host, sessh fails

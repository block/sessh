...
`sessh` starts a daemon namespace it writes `sesshd`, `sessh-broker`, and
`sessh-proxy` symlinks beside `sesshd.sock`, all pointing back to the active
`sessh` binary. That keeps packaged archives simple while still making `ps`
output readable.

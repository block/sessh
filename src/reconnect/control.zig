...
pub fn scanInput(bytes: []const u8, options: ScanOptions) InputAction {
    for (bytes) |byte| {
        if (options.ctrl_c_detaches and byte == ctrl_c) return .detach;
...
test "scanInput recognizes reconnect controls only when requested" {
    ...
    try @import("std").testing.expectEqual(InputAction.detach, scanInput(&.{ctrl_c}, .{ .ctrl_c_detaches = true }));
}

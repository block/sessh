...
pub const Frame = struct {
    ...
    pub const payload_union = union(_payload_case) {
        ...
        stream_parent_status: StreamParentStatus,
        stream_reconnect_now: StreamReconnectNow,
        pub const _desc_table = .{
            ...
            .stream_resume = fd(34, .submessage),
            .stream_data = fd(35, .submessage),
            ...
            .stream_parent_status = fd(41, .submessage),
            .stream_reconnect_now = fd(42, .submessage),
        };
    };
...
/// Sent at the start of every ssh transport that carries a reconnectable byte
/// stream. The receiver uses recv_next_offset to drop bytes that the peer
/// already received and resume from the first unacknowledged byte.
pub const StreamResume = struct {
...
/// Raw bytes from this peer's outbound stream.
...
/// Acknowledges received StreamData bytes.
...
/// Marks the end of this peer's outbound stream.
pub const StreamEof = struct {
    offset: u64 = 0,
...
/// Acknowledges that StreamEof was received.
pub const StreamEofAck = struct {
    offset: u64 = 0,
...
pub const StreamParentRetry = struct {
    delay_ms: u64 = 0,

    pub const _desc_table = .{
        .delay_ms = fd(1, .{ .scalar = .uint64 }),
    };
...
pub const StreamParentReconnecting = struct {
    pub const _desc_table = .{};
...
pub const StreamParentClear = struct {
    pub const _desc_table = .{};
...
/// Sent by the stream ProxyCommand to its parent sessh process. ProxyCommand
/// stdout is reserved for the ssh byte stream, so reconnect UI is requested over
/// the same framed protobuf protocol on the local parent-control socket.
pub const StreamParentStatus = struct {
    ...
    pub const status_union = union(_status_case) {
        retry: StreamParentRetry,
        reconnecting: StreamParentReconnecting,
        clear: StreamParentClear,
        pub const _desc_table = .{
            .retry = fd(1, .submessage),
            .reconnecting = fd(2, .submessage),
            .clear = fd(3, .submessage),
        };
    };
...
/// Sent by the parent sessh process to the stream ProxyCommand when the user
/// asks to reconnect immediately from local reconnect UI, currently with
/// Ctrl-R. This is intentionally outside the ssh byte stream so the remote
/// process never receives the UI control byte.
pub const StreamReconnectNow = struct {
    pub const _desc_table = .{};

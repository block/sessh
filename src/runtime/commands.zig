...
pub fn killTargetsForJsonl(allocator: std.mem.Allocator, targets: []const []const u8) ![]KillResult {
    var results: std.ArrayList(KillResult) = .empty;
...
pub fn requestKillTargetsNoWait(allocator: std.mem.Allocator, targets: []const []const u8) void {
    for (targets) |target| requestKillTargetNoWait(allocator, target) catch {};
}

pub fn anyLiveSessionExists(allocator: std.mem.Allocator) bool {
    const sessions_dir = session_registry.sessionsDir(allocator) catch return false;
    defer allocator.free(sessions_dir);
    var dir = std.fs.openDirAbsolute(sessions_dir, .{ .iterate = true }) catch return false;
    defer dir.close();

    var iterator = dir.iterate();
    while (iterator.next() catch return false) |entry| {
        if (entry.kind != .directory or !session_registry.isValidSessionId(entry.name)) continue;
        var paths = session_registry.pathsForSessionId(allocator, entry.name) catch continue;
        defer paths.deinit(allocator);
        var meta = session_registry.readMeta(allocator, paths) catch {
            session_registry.removeStaleHints(paths) catch {};
            continue;
        ...
        defer meta.deinit(allocator);
        if (processExists(meta.agent_pid)) return true;
        session_registry.removeStaleHints(paths) catch {};
    }
    return false;
}
...
fn requestKillTargetNoWait(allocator: std.mem.Allocator, target: []const u8) !void {
    if (session_registry.isValidProxyGuid(target)) return requestProxyTargetNoWait(allocator, target);
    return requestSessionTargetNoWait(allocator, target);
}
...
fn requestSessionTargetNoWait(allocator: std.mem.Allocator, session_id: []const u8) !void {
    var paths = pathsForLocalSessionRef(allocator, session_id) catch return;
    defer paths.deinit(allocator);
    ...
    _ = signalProcess(meta.agent_pid, c.SIG.TERM);
}
...
fn requestProxyTargetNoWait(allocator: std.mem.Allocator, guid: []const u8) !void {
    var socket_paths = session_registry.runtimeAgentSocketPathsForGuid(allocator, guid) catch return;
    defer socket_paths.deinit(allocator);
    if (!pathExists(socket_paths.socket)) {
        socket_paths.removeRuntimeFiles();
    ...
    _ = signalProcess(agent_pid, c.SIG.TERM);
}
...
test "parseKillRequestJson accepts kill request payload" {
    const request_json = "{\"guid\":\"s-00000000-0000-4000-8000-000000000001\",\"requested_age_ms\":123}";
    var request = try parseKillRequestJson(std.testing.allocator, request_json);
    defer request.deinit(std.testing.allocator);

    try std.testing.expectEqualStrings("s-00000000-0000-4000-8000-000000000001", request.guid);
    try std.testing.expectEqual(@as(u64, 123), request.requested_age_ms);
    try std.testing.expectError(error.InvalidKillArgs, parseKillRequestJson(std.testing.allocator, "{\"guid\":\"x\",\"requested_age_ms\":123}"));
}

...
pub const FrameForwarder = struct {
    ...
    /// Reads at most one chunk from `read_fd`. The frame envelope is preserved
    /// exactly: length header first, then exactly that many body bytes. The body
    /// is streamed through the fixed circular buffer and is never decoded or
    /// allocated.
    pub fn readReady(self: *FrameForwarder) !FrameForwarderReadStatus {
        ...
        if (self.header_filled < protocol.frame_header_len) {
            ...
            switch (status) {
                ...
                .filled => {
                    self.frame_remaining = protocol.payloadLenFromHeader(&self.header);
                    return .progress;
                },
                .eof_empty => {
